...
//!   to `chapter-one.md`.
//!
//! Both run over `Entry::name_folded`, lowered once during the walk.
//!
//! CJK note: the subsequence matcher steps over `char`, not bytes, and the
//! returned positions are character offsets, not byte offsets — the renderer
//! highlights by character, and a byte offset lands mid-character in any CJK
//! name. Ported from legacy `packages/fs/src/search.rs`.

use memchr::memmem;
use rayon::prelude::*;

use super::index::{Entry, Kind};
...
/// Score components. The ranking is a product decision, and a reviewer should
/// be able to read the weights without reading the loop.
mod weight {
...
mod tests {
    ...
    fn cjk_positions_are_character_offsets_not_bytes() {
        ...
        // Byte offset would be 3; character offset is 1. The renderer
        // highlights by character, so a byte offset underlines the wrong glyph.

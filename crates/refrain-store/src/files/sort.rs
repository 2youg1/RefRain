...
//! them by name makes a tree unreadable. Every order keeps the split, in both
//! directions.
//!
//! **Natural order for names.** `chapter-10.md` belongs after `chapter-9.md`,
//! not between `chapter-1` and `chapter-2`. Ported from legacy
//! `packages/fs/src/sort.rs`.
...
///
/// Leading zeros do not change the value, so `01` and `1` compare equal and
/// fall through to the tie-break.
...
/// Consume a run of digits and return its value. Saturating: a hundred-digit
/// filename must not overflow into a wrong order or a panic.
...
/// Directories before files, symlinks last.

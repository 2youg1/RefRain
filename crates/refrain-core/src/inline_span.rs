...
//! **Rust 出结构，视图层出像素**。
//!
//! # 这份解析器与 TypeScript 那份的分工
//!
//! `packages/editor/src/inline-render.ts` 里有一份**看起来做同一件事**的解析器。
//! 它们不是重复，消费者不同，而且各自的消费者都不能用对方那一份：
//!
//! | | 这里（Rust） | `inline-render.ts`（TS） |
//! |---|---|---|
//! | 消费者 | 搜索索引 | 编辑视图渲染 |
//! | 出口 | [`strip_inline_markers`] 去掉标记符 | 扁平切片，标记符留着画淡 |
//! | 在哪跑 | `ensure_indexed`，前端不参与 | `#paintText`，浏览器里 |
//! | 为什么不能用对方 | 渲染必须**同步**——`#paintText` 之后紧跟 `placeCaret`，走 Tauri 异步 IPC 会让光标在标记重算完成前落位，作者每敲一字光标闪跳 | 索引跑在 Rust 侧的 SQLite 事务里，那里没有 JavaScript 运行时 |
//!
//! 两份实现不共享代码，但**必须给出一致的判定**：搜索剥掉的标记与渲染画淡的
//! 标记若是两套，作者就会看到「屏幕上是粗体、搜索却按字面匹配」这种矛盾。
//! `packages/editor/test/inline-render-parity.test.ts` 用同一批语料对拍两侧，
//! 语料表在那个文件里，改动任一侧的配对规则都会让它变红。
//!
//! 还有第三份 `inline-mark.ts`，那**不是**解析器：它回答「作者按 Ctrl+B 时这个
//! 选区该改写成什么」，扫的是一个选区而非全块，出口是新的源文本而非区间。
//! 同一套语法的三种问法，各自的出口类型就是它们不能合并的证据。

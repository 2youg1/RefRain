...
| Q22 | Where do application state and the immutable Source Backup live when the Root is one Markdown file rather than a directory? | Open — `join(root, ".refrain")` and `readdirSync(root)` both treat the file as a directory and fail with `ENOTDIR`. Storing beside the file risks adopting neighbouring manuscripts; storing under global user data weakens project portability. No implementation may silently choose either model before this is decided. |
| Q23 | Should failure to take a Source Backup block folder-Root adoption? | Open — the current path keeps the manuscript editable and returns the concrete failure through the existing Notice. Silent degradation is forbidden. Before v1, decide whether preserving access or guaranteeing an original takes precedence when the disk refuses the safety copy. |
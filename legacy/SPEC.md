...
- 2026-07-28 v0.1.6 — Closed Q27 and completed Q16's focus contract. The main process now binds a queued Review Task atomically to the current Text Head and copies its readable Context Scope from that same head; the renderer submits an intent, never a claimed Revision. Whole-chapter review carries one material Context Scope and no Edit Scope, while selected work keeps the selection as the only writable scope. A Sheet's opening chain supplies a connected return-focus target before a Palette or context menu disappears; the Sheet no longer guesses ownership from whichever node happens to be focused after unmount.

- 2026-07-27 v0.1.6 — Root permission moved to the main process. Only a picker, drop, create, OS-open event, or a permit already stored in main-owned application state may authorize a Root; renderer localStorage only remembers presentation. Each permit pins the canonical path, filesystem identity, and an immutable nonce in the Root's application state; a single-file Root pins its parent so RefRain's own atomic save remains valid. Retargeting or replacing a Root revokes mutation authority, and Source Backup cannot receive a permit.
...
| **Text Head** | The immutable manuscript state produced by a completed Text Action. Exactly one is current. Its ID is opaque: new IDs come from one core-owned local UUID authority; historical non-empty strings remain readable |
...
The user either selects a writable range or leaves the selection empty for comment-only whole-chapter work, writes a prompt, picks an agent, and adds it to a pending queue. The queue accumulates across chapters and agents, then goes out on **one click with one consolidated manifest**.

Idle time, cursor movement, and autosave never create a Review Task.

Before sending, per agent: run count, locked harness, locked model (tagged whether verified), Context Scope identities, Edit Scope ranges or their explicit absence, prompt text. **No prices. No cost estimates.**

Queueing crosses one authority boundary. The renderer supplies the chapter, prompt, agent, and any selected Edit Scopes; the main process reads the current Text Head, verifies each selected scope's block IDs and before-text against it, and in the same synchronous operation binds the Review Task to that Head's ID and copies the readable Context Scope. A stale selection is refused rather than attached to a newer Revision. Whole-chapter inspection carries one material Context Scope containing the chapter at that Revision and no Edit Scope; its result may contain comments targeted at that context, but no replacement. Persisted tasks from the former ID-only Context Scope remain readable as legacy references and are labeled unavailable rather than presented as empty material.
...
**INV-10 · Text Head identity has one authority.** Every new Text Head ID comes from one core-owned local UUID generator. A load, a Text Action, and a desktop fallback do not derive identity from a resettable counter, path, chapter name, or wall clock. Readers treat persisted IDs as opaque non-empty strings, so historical `hN`, `path@load`, timestamp, and dispatch-baseline values are not rewritten or rejected. Gate: `packages/core/test/text-head-id.test.ts`, including two cold runtime graphs and repeated loads.

**INV-11 · Revision binding and Context Scope share one Head.** The renderer cannot choose a Review Task baseline or supply readable context as an unsupported claim. At enqueue, the main process binds the current Text Head ID and material copied from that same immutable Head, after verifying every Edit Scope against it. A whole-chapter comment task has one readable Context Scope and zero Edit Scopes. A comment may target either a supplied Context Scope or Edit Scope; a replacement may target only an Edit Scope. Gate: desktop agent IPC tests plus the real Dispatch browser gate.
...
| Q10 | Which Japanese faces ship, alongside which Chinese ones? | Closed — 2026-07-27, **restated 2026-07-28 against what is actually in the package.** Three slots rather than two: Chinese, Japanese, and Latin are separate settings, because one CJK slot cannot serve both a Chinese and a Japanese reader — 直, 骨, and 令 are drawn differently and a single face gets one of them wrong. Japanese carries a mincho as well as a gothic, because Japanese body text is set in mincho and a gothic there is a display face. **Chinese serif: Chiron Sung HK. Chinese sans: Noto Sans SC. Japanese mincho: Shippori Mincho. Japanese gothic: Zen Kaku Gothic New, with Murecho for the interface. Latin: Antic Didone, Jost, Courier Prime.** All eight are SIL OFL 1.1 with no reserved name, so none needs renaming on subset; `apps/desktop/src/renderer/fonts/LICENSES.md` carries the per-face attribution and `BUNDLED_CJK` / `BUNDLED_JP` / `BUNDLED_LATIN` in `typography.ts` are the code-side authority. The earlier answer named KazukiReiwa (shipped subset renamed `RefRain Mincho`) and ChillDINGothic; **neither was ever packaged.** Both carry reserved names, which is the cost this choice avoided, and this entry stood contradicting the package for long enough that it is worth naming: a closed question is not a shipped fact. Rejected: 致一黑體_傳承形 and Mizuki-Gothic are IPA Font License — legal to ship only byte-for-byte, unrenamed, unsubsetted, and as a standalone file rather than linked into the executable; MiSans and OPPO Sans forbid redistributing the font binary at all. |
...
| Q16 | How do Stage, Reference and Safety differ? | Closed — 2026-07-27; focus contract completed 2026-07-28. **By lifetime, not by appearance.** Stage is the work in hand and persists across sessions. Reference is consulted and closed, holding nothing. Safety interrupts and is the only one permitted to be modal — a conflict the author must resolve before the manuscript can move. Review and Dispatch stay drawers, separate from settings: settings is a Reference and must not evict work in progress by sharing its slot. The App owns each drawer's return path: a direct control returns to itself, a Palette command returns to the persistent Ctrl-K control, and a context-menu command returns to the manuscript. The opening chain passes that connected target before the transient control disappears; the Sheet does not infer ownership from global focus or reopen a vanished menu. |
...
| Q25 | When is a persisted Root permission checked, and what happens when the Root drifted? | Closed — 2026-07-27. **Permission and filesystem identity are separate facts, but neither renderer memory nor a familiar pathname proves authority.** A permit lives in user-controlled application state (`userData/roots.json`) and pins canonical path, Root kind, device, inode, birth timestamp, and an immutable project-state nonce; a single-file Root pins its parent so RefRain's own atomic save may replace the file. Workspace loading distinguishes an ungranted path from a granted one that is missing, but verifies every present Root before loading can create project state or Source Backup. A missing Root stays visible and quiet; a replaced or retargeted Root is refused with a concrete reason. Every later rooted operation rechecks identity. RefRain reports and does not repair: there is no second copy, and pretending to restore one would be worse than saying so. |
| Q26 | What identifies a Text Head across workbenches and restarts? | Closed — 2026-07-28. **One core-owned, offline UUID authority mints every new Text Head ID.** The ID is an opaque key, not a path, timestamp, sequence, or validity proof. Existing persisted strings remain readable and are never rewritten merely to match the new spelling; only new production constructors must use the authority. No network or cross-Root registry is introduced. |
| Q27 | Who binds a Review Task to a Revision, and what makes Context Scope readable? | Closed — 2026-07-28. **The main process binds both from one current Text Head at enqueue.** The renderer submits the chapter, agent, prompt, and optional Edit Scopes; it cannot submit a baseline or manuscript context. Main verifies every writable scope against the Head, uses that Head ID as the Revision, and copies the whole chapter into a named material Context Scope. With no selection, the task remains valid with zero Edit Scopes and can produce targeted comments but no replacement. HostState v2 stores material contexts as `{ kind: "material", id, text }`; v1 ID-only contexts migrate to `{ kind: "legacy-reference", id }` and are reported unavailable rather than fabricated. |
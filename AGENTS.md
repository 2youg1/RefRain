...
Tags run the release matrix on Windows x64, Linux x64, macOS arm64, and macOS x64. Each runner builds and tests its own N-API binary before packaging. The matrix uploads installers; one final job publishes five files: NSIS, AppImage, deb, and two DMGs. A release is incomplete until that GitHub Actions run is green and all five files exist.

...
// 步骤 10 之后色表只剩一份产物（Zig），所以「几套主题」的权威是它，
// 而 core 的 THEME_COUNT 必须跟着它。两处漂开的表现是换主题时越界回落，
// 而两边单看都自洽。
...
const declared = core.match(/const THEME_COUNT = (\d+);/u)?.[1];
if (declared === undefined || Number(declared) !== count) {
  console.error(
    `FAIL  verify:themes-current: core declares THEME_COUNT ${declared ?? "nothing"}, the table has ${count}`,
  );
  process.exit(1);
}
...
const coreDefault = core.match(/const DEFAULT_THEME_INDEX = (\d+);/u)?.[1];
if (defaultIndex === undefined || coreDefault === undefined || defaultIndex !== coreDefault) {
  console.error(
    `FAIL  verify:themes-current: default theme index disagrees (native ${defaultIndex ?? "none"}, core ${coreDefault ?? "none"})`,
  );
  process.exit(1);
}

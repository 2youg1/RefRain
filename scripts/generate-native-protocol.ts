...
/**
 * Generated output is Source Code Form too, so it carries Exhibit A like every
 * hand-written file. The notice leads and the `@generated` marker follows it:
 * one file-head shape across the tree lets `verify:licence-headers` hold a
 * single predicate with no exception list.
 */
const SLASH_NOTICE = noticeBlock("slash");
const BLOCK_NOTICE = noticeBlock("block");

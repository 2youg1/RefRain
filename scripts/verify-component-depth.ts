...
const BODY_DEBT: Readonly<Record<string, number>> = {
  ...
  // 635：Cmd+1..4 层导航（quarter-navigation.ts，17 项测试）。四区的规矩此前
  // 七个出口零调用——写下来了但渲染层没人守，而它们越晚接线越会与后续改动漂开。
  //
  // 净增的是外壳提供的五个执行动作（开设置、聚焦侧栏、回正文、开派发、开面板）。
  // 「解析键→查规矩→记忆→分派」整条已收进 `runQuarterKey`，外壳不认识
  // `quarterForKey` 也不认识那五个分支。第一次搬完是 646，抽出 runQuarterKey
  // 后 634——**跟棘轮较劲仍会净增**（这是第五次），剩下的是真接线。
  //
  // 数值取自 bun run fmt 之后：格式化会改行数，格式化之前登记会被下降分支咬住。
  "Workbench.tsx": 635,

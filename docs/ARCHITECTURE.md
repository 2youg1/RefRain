...
|---|---|
| **No network** | The application process opens no sockets. Three readings, none of which replaces another: `verify:no-network` scans the code we wrote and the resolved dependency graph in `Cargo.lock`; `verify:no-network-imports` reads the shipped binary's import table and holds it to an exact register. The graph reading exists because a repository that never wrote `fetch` can still reach the network through a dependency. The artifact reading exists because both of the others are upstream of the only object an author runs — and it found that the binary binds the whole Winsock surface plus WinHTTP, all of it from `ws2_32` (linked unconditionally by the app build) and from Media Foundation (linked by the SDK for video). The invariant is about what the process *does*; the register is what keeps that surface from growing without anyone saying why. |
...
| `view/files.zig` | The file tree on the SDK's `virtualList`, the row menu, and the open/delete/disclose/import Msgs. `tree_window` is the one declaration both the window solve and the build read. `borrowDocumentReference` is the one account of what a tree row lends to `document_open` — the search hits borrow through the same call | the `files` journal; in-file tests on the borrow and on the built tree (A1, A2) |
...
| `core/replies.zig` | Where a project reply lands so it outlives the call that delivered it — 14 slots, chosen by the reply's own `kind`, so reading the settings does not wipe the block listing. All 14 are the size of the ABI bound, and deliberately: a slot's maximum is its rows **plus the text they point at**, and no kind bounds that text below the ABI bound. Same discipline as `host_bridge.zig`'s projection buffer, and the same reason | in-file tests, including one that fails if any slot is narrowed |
...
| The panel material an author chose | `apps/native/src/material.zig` (`Kind`, `kebabOf`, `kindFromKebab`, `recipe`) | A material index. `Model.panel_material` carries the `Kind` itself, so no reader has an out-of-range case to answer and no view has a fallback arm to write. A fourth density is a compile error at the recipe table and at `kebabOf` |
...
| **Application shell** | [Native SDK](https://native-sdk.dev) `@native-sdk/cli`, at the version `bun.lock` resolves, with an increment in `patches/`. Native rendering. No WebView and no JavaScript runtime in the binary |
...
`patches/` is not a private fork. It is an increment on `@native-sdk/cli` at the
version `bun.lock` resolves — stated there and nowhere else, because bun applies
a patch only when the `patchedDependencies` key matches that version exactly and
otherwise applies nothing in silence. `verify:patched-dependency` holds the two
together; the patch file carries no version in its name for the same reason.

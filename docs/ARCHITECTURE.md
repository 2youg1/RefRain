...
| `core.zig` | The Zig core: `initFx` and `update`. 34 of the 74 arms are migrated; `pending_arms` names the rest, and a test proves each name is a real arm, so the remaining work is countable | in-file tests |
...
| `core/model.zig` | The whole interface state: 30 top-level fields against the budget of 40, with a test that fails if a field is added past it | in-file tests |

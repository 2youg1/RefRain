...
app.whenReady().then(() => {
  win = new BrowserWindow({
    x: 60, y: 60, width: 1100, height: 800, alwaysOnTop: true,
  ...
  win.webContents.on('did-finish-load', () => {
    setInterval(async () => {
      try {
        ...
        const ready = await win.webContents.executeJavaScript('!!(window.__ime && window.__ime.ready)');
      ...
      } catch (e) { /* window navigating/closing */ }
    }, 1000);
  });
});

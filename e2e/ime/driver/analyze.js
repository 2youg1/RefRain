// analyze.js: merge results/<shell>/final.json into results/summary.md
import { readFileSync, writeFileSync, existsSync } from 'node:fs';

const ROOT = new URL('..', import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1');
const shells = ['e42', 'e43', 'e44', 'wv2'];
const PUNCT = { ',': '，', '.': '。', '?': '？', '!': '！' };

function sliceByMarks(rep, from, to) {
  const m = Object.fromEntries(rep.marks.map(x => [x.name, x.t]));
  const t0 = m[from] ?? 0, t1 = m[to] ?? Infinity;
  return { t0, t1, evs: rep.events.filter(e => e.t >= t0 && e.t < t1) };
}

function analyze(shell) {
  ...
  // phase 3: punctuation (IME-processed keys surface as keydown key==='Process')
  const p3 = sliceByMarks(rep, 'phase3-punct', 'end');
  const punctKeys = p3.evs.filter(e => e.type === 'keydown' && (PUNCT[e.key] || e.key === 'Process'));
  const CH_PUNCT = ['，', '。', '？', '！'];
  let firstPressOK = 0;
  const fails = [];
  const perChar = {};
  for (const k of punctKeys) {
    const hit = p3.evs.find(e =>
      e.t > k.t && e.t - k.t < 900 &&
      ((e.type === 'beforeinput' && e.inputType === 'insertCompositionText' && CH_PUNCT.includes(e.data)) ||
       (e.type === 'beforeinput' && CH_PUNCT.includes(e.data)) ||
       (e.type === 'compositionend' && CH_PUNCT.includes(e.data))));
    if (hit) { firstPressOK++; perChar[hit.data] = (perChar[hit.data] || 0) + 1; }
    else fails.push({ key: k.key, t: k.t });
  }

  // phase 1: latency from phase1 mark to first compositionstart
  const m1 = rep.marks.find(x => x.name === 'phase1-first-word');
  const cs1 = m1 ? rep.events.find(e => e.t >= m1.t && e.type === 'compositionstart') : null;

  // freeze sentinels overall
  const stuckComps = rep.comps.filter(c => c.start != null && c.end != null && c.end - c.start > 3000).length;
  const openComp = rep.openComp && rep.openComp.age > 3000 ? 1 : 0;

  return {
    shell,
    chrome: rep.chrome,
    electron: (rep.ua.match(/Electron\/[\d.a-z-]+/) || ['-'])[0],
    ...
    firstClickToCompositionMs:
      (rep.firstPointerDown != null && rep.firstCompositionStart != null)
        ? rep.firstCompositionStart - rep.firstPointerDown : null,
...
md += line('标点分布', r => Object.entries(r.punctPerChar).map(([k, v]) => `${k}×${v}`).join(' '));

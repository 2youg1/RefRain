...
param(
  [Parameter(Mandatory=$true)][ValidateSet('wv2')][string]$Shell,
  [string]$Root = '',
  [int]$BurstSeconds = 60
)

$ErrorActionPreference = 'Stop'
# $PSScriptRoot is empty inside a param default (PowerShell 5.1): resolve the
# default here, in the body, where it is set.
...
public static class Win {
  ...
  [DllImport("user32.dll")] public static extern bool AttachThreadInput(uint attach, uint attachTo, bool attach_);
  [DllImport("kernel32.dll")] public static extern uint GetCurrentThreadId();
  public struct RECT { public int Left, Top, Right, Bottom; }
}
"@

# Windows only lets a process take the foreground under input-privilege rules a
# background launcher never satisfies. Attaching both input queues first makes
# SetForegroundWindow legal everywhere the harness runs (CI agent session,
# a terminal that owns the foreground, a locked-down desktop).
function Force-Foreground([IntPtr]$hwnd) {
  $fg = [Win]::GetForegroundWindow()
  $nullPid = [uint32]0
  $fgThread = [Win]::GetWindowThreadProcessId($fg, [ref]$nullPid)
  $targetThread = [Win]::GetWindowThreadProcessId($hwnd, [ref]$nullPid)
  $current = [Win]::GetCurrentThreadId()
  [Win]::AttachThreadInput($current, $targetThread, $true) | Out-Null
  [Win]::AttachThreadInput($fgThread, $targetThread, $true) | Out-Null
  [Win]::SetForegroundWindow($hwnd) | Out-Null
  [Win]::AttachThreadInput($current, $targetThread, $false) | Out-Null
  [Win]::AttachThreadInput($fgThread, $targetThread, $false) | Out-Null
}
...
while (-not (Test-Path "$outDir\ready.flag")) {
  if ((Get-Date) -gt $deadline) {
    Write-Host "child alive: $(($null -ne (Get-Process -Id $proc.Id -ErrorAction SilentlyContinue)))"
    Write-Host "outDir: $outDir"
    Get-ChildItem $outDir -ErrorAction SilentlyContinue | ForEach-Object { Write-Host "  has: $($_.Name)" }
    if (Test-Path "$outDir\host.log") { Get-Content "$outDir\host.log" | ForEach-Object { Write-Host "  host.log: $_" } }
    throw "ready.flag timeout for $Shell"
  }
...
# --- find window
# Pick the window of the host this run launched. Zombie hosts from earlier
# runs share the IME-TEST* title, and clicking a zombie feeds every keystroke
# to a window nobody is watching.
$hwnd = [IntPtr]::Zero
$deadline = (Get-Date).AddSeconds(20)
while ($hwnd -eq [IntPtr]::Zero) {
  try { $hwnd = (Get-Process -Id $proc.Id -ErrorAction Stop).MainWindowHandle } catch {}
  if ($hwnd -ne [IntPtr]::Zero) { break }
  if ((Get-Date) -gt $deadline) { throw "the launched host has no window" }
  Start-Sleep -Milliseconds 500
}
$title = (Get-Process -Id $proc.Id).MainWindowTitle
if ($title -notlike 'IME-TEST*') { throw "launched host window title is '$title', expected IME-TEST*" }

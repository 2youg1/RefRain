 ...
 *
 * 超椭圆（squircle）`|x/a|^n + |y/b|^n = 1` 让曲率**长出来**而不是跳上去。
 *
 * ## 连续阶由指数决定，不是「够方就叫 G4」
 *
 * 拐角与直边相接处，曲率沿弧长的行为是 **κ ∝ s^(n−2)**，而连续性的阶数取决于
 * κ 从第几阶导数开始跳变：
 *
 * | n | κ 的行为 | 从哪一阶跳变 | 阶 |
 * |---|---|---|---|
 ...
 * | 5 | κ ∝ s³ | κ‴ | **G4** |
 *
 * 所以 **G4 要 n = 5**。此前这里写着「n ≈ 4~5 就是 G4」，那是把「拐角够顺」
 * 当成了阶数的判据——n = 4.2 实际约 G3.2，一个非整数阶。名字要么给得准，
...
type CornerSpec = {
  ...
  /**
   * 超椭圆指数。它决定连续阶：κ ∝ s^(n−2)，n = 5 才是 G4。
   *
   * 各档取值按「这一档需要到哪一阶」来定，不按「看起来够不够顺」——后者
   * 是感受，前者可以被门禁验证。
   */
...
const SPECS: Record<CornerScale, CornerSpec> = {
  // 小饭盒停在正文旁边，与正文那条直边并排——曲率突变在这种并排关系里
  // 最显眼，所以它是唯一需要 G4 的一档，取 n = 5。
  bento: { radius: 12, exponent: 5 },
  // 面板占一整条竖带，它的角旁边没有别的直边可比，G3 足够。
  panel: { radius: 10, exponent: 4 },
  // 列表里的一格：数量多、尺寸小，G2 已经看不出突变。
  card: { radius: 7, exponent: 3 },
  control: { radius: 5, exponent: 3 },
 ...
 * 这一档的曲率连续阶。
 *
 * κ ∝ s^(n−2)，κ 从第 (n−2) 阶导数开始跳变，所以阶数是 n − 1。非整数指数
 * 给出非整数阶（n = 4.2 → 约 G3.2），如实返回而不取整——取整会把「差一点」
 * 说成「到了」。
 */
export function cornerContinuity(scale: CornerScale): number {
  return SPECS[scale].exponent - 1;
}

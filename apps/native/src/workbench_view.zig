...
//! 越界怎么落）住在 `core/workbench.zig`，可在 Null platform 上单测；
//! 这里只补它答不了的那一半——中文标签。
//!
...
//! 新增一个去处只改这张表与 `core/workbench.zig` 的两个常量，视图不必动。
...
/// 八个去处，次序与 `core/workbench.zig` 的 `Destination` 严格一致。
///
...
/// 这个下标的去处。越界回落到稿子——与 `core/workbench.zig` 的 `destinationAt`
/// 同规。
///
/// 两处各自钳一次不是重复：核心那侧钳的是 Model 的值，这里钳的是查表的
/// 下标，而视图不能假设 Model 一定守规矩（它跨了一次 ABI）。
...
/// 直达这个去处的键位：`core/workbench.zig` 的 `destinationForOrdinal` 的反查
...
test "the destination table matches the core's index vocabulary" {
    // `core/workbench.zig` 的 `Destination` 有 8 个。两处漂开的表现是命令面板
    // 少一行或多一行空条目，而两边单看都自洽。
...
test "destination chords are the inverse of destinationForOrdinal" {
    // Ctrl+1 是设置、Ctrl+2 文件、Ctrl+3 稿子（core/workbench.zig 的 ordinal
